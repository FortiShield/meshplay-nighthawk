// Copyright 2024 The Meshplay Authors
//
...
import (
	...
	"github.com/khulnasoft/meshplay-adapter-library/api/grpc"
	configprovider "github.com/layer5io/meshkit/config/provider"
	"github.com/layer5io/meshkit/logger"
	"github.com/layer5io/meshkit/utils/events"
	"github.com/meshplay/meshplay-nighthawk/build"
	"github.com/meshplay/meshplay-nighthawk/internal/config"
	"github.com/meshplay/meshplay-nighthawk/nighthawk"
	"github.com/sirupsen/logrus"
)

var (
	serviceName = "meshplay-nighthawk"
	version     = "edge"
	gitsha      = "none"
...
func meshplayServerAddress() string {
	meshReg := os.Getenv("MESHPLAY_SERVER")
...
func registerCapabilities(port string, log logger.Handler) {
	// Register workloads
	log.Info("Registering static workloads with Meshplay Server...")
	if err := adapter.RegisterMeshModelComponents(instanceID, meshplayServerAddress(), serviceAddress(), port); err != nil {
		log.Error(err)
	}

	log.Info("Successfully registered static components with Meshplay Server.")
}
...
func registerWorkloads(port string, log logger.Handler) {
	log.Info("Registering latest components with Meshplay Server")
